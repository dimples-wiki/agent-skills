...
1. 寓言作为**末章**插入已生成页面：放在**自测块**（`<section ... data-quiz>`）
   **之后**、**数字账本**（`<details ... data-ledger>`）**之前** —— 全页最后一个
   section，账本仍是页尾附录。自测块与账本保留不动（门 12 / 门 13 要求它们存在）；
   寓言若引入了新的 N% / N 倍 数字，也要入账（门 13 扫全页正文）。
